...
export function ManualDeliveryOrderModal({ lojaId, onClose, onCreated }: {
  ...
  useEffect(() => {
    api.get(`/lojas/${lojaId}`).then((response) => setStore(unwrap(response))).catch(() => setStore(null));
  }, [lojaId]);
